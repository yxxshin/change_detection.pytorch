...
class UnetPlusPlusDecoder(Decoder):
    def __init__(
        ...
        for layer_idx in range(len(self.in_channels) - 1):
            for depth_idx in range(layer_idx+1):
                ...
                in_ch = self.skip_channels[layer_idx - 1] if depth_idx == layer_idx and depth_idx != 0 else self.in_channels[layer_idx]
        ...
        self.seg_ensemble = seg_ensemble
        self.ECAM = md.ECAM(in_channels=self.out_channels[-2], out_channels=self.out_channels[-1]) if seg_ensemble == "ecam" else None
    ...
    def forward(self, *features):
        ...
        if self.seg_ensemble == "ecam":
            return self.ECAM([dense_x[f'x_{i}_{self.depth - 1}'] for i in range(self.depth)])
        else:
